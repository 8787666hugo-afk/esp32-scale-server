...
步驟1 找出該月的「無賽事基準」
      = 歷史上同一個月份、且該月「完全沒有任何事件」的那一年的實績。

      判定方式（兩個都要查，不可只憑印象）：
        (a) 該年月不可出現在事件清單裡 —— 不論正面或負面事件都不行。
            經銷商倒閉、匯率、缺料這種負面事件也會扭曲基準。
        (b) 該年月不可出現在賽事行事曆裡。

      有多年符合時取最近一年。
      reason 必須寫「已確認 20XX-XX 無事件」，把查證結果講出來。

      ⚠ 只有在同月份的每一年都有事件時，才可以退而取歷史最小值，
        並在 reason 中註明「同月各年皆有事件，改取最小值」。
        歷史資料不足、看不到該月份時，不可以硬取其他月份的最小值當基準
        —— 那會拿一個不相干月份的數字去乘賽事倍率。這種情況
        adjustment 一律填 0，reason 寫「歷史資料不足，無法計算」。
